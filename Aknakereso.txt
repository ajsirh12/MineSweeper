...
int foglal (int ***t, int sor, int oszlop) {
	...
	/*Parameterek:a tömb cime(nullpointer), merete.
...
int indit (int ***t, int sor, int oszlop, int akna) {
	/*Tombinicializalo.
	A tomb elemeit kinullazza, majd veletlenszerüen kitolti aknakat jelento 'B' karakterekkel(ASCII).
...
int vizsgal(int **t, int sor, int oszlop, int y, int x) {
	...
	a tapogat() fv. segítségével azt is, hogy hány szomszédos mezon van.
...
void kiir(int **t, int sor, int oszlop, int akna) {
	...
	hamis: csak a szomszédos aknak szamat jelzo szamok jelennek meg*/
	int x, y;
	if (akna) {
		putchar(' ');
		putchar(' ');
